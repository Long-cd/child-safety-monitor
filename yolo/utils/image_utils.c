...
// 修复后的 read_image_stb 函数（读取PNG等格式）
static int read_image_stb(const char* path, image_buffer_t* image)
{
    if (image == NULL || path == NULL) { // 增加参数校验
        printf("error: invalid param, image or path is NULL\n");
        return -1;
    }

    // 默认图像为3通道，stbi_load会自动解析PNG的通道数
    int w, h, c;
    unsigned char* pixeldata = stbi_load(path, &w, &h, &c, 0);
    if (!pixeldata) {
        printf("error: read image %s fail\n", path);
    ...
    // ========== 修复1：内存拷贝前增加大小检查，避免越界 ==========
    if (image->virt_addr != NULL) {
        // 确保目标内存足够容纳像素数据
        if (image->size < size) {
            printf("error: image->size (%d) < pixel data size (%d)\n", image->size, size);
            stbi_image_free(pixeldata);
            return -2;
        }
        memcpy(image->virt_addr, pixeldata, size);
        stbi_image_free(pixeldata); // 拷贝后释放stb分配的内存
    } else {
        // 直接复用stb分配的内存
        image->virt_addr = pixeldata;
    }
    ...
    image->width = w;
    image->height = h;
    image->size = size; // 关键：赋值数据大小
    // PNG读取后步长无额外对齐，步长等于宽/高
    image->width_stride = w;
    image->height_stride = h;
    // 根据通道数设置格式
    if (c == 4) {
        image->format = IMAGE_FORMAT_RGBA8888;
    } else if (c == 1) {
    ...
    // stb库不使用文件描述符，设为-1区分默认的0
    image->fd = -1;

    printf("success: load image %s, wxhxc=%dx%dx%d, size=%d bytes\n", path, w, h, c, size);
    return 0;
}
...
int convert_image(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    int ret;
    //===========================================
    // 强制禁用RGA，直接用CPU（注释掉原有条件编译逻辑）
    printf("convert image use cpu (force disable RGA)\n");
