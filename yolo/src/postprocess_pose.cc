...
#define OBJ_CLASS_NUM POSE_OBJ_CLASS_NUM
#define NMS_THRESH    POSE_NMS_THRESH
#define BOX_THRESH    POSE_BOX_THRESH
...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "/home/linaro/projects/yolo_test/model/yolov8_pose_labels_list.txt"
...
int pose_post_process(rknn_pose_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold,
                 pose_detect_result_list *od_results) {
    ...
    memset(od_results, 0, sizeof(pose_detect_result_list));
    int index = 0;
#ifdef RKNPU1
    for (int i = 0; i < 3; i++) {
