...
#define OBJ_CLASS_NUM SEG_OBJ_CLASS_NUM
#define NMS_THRESH    SEG_NMS_THRESH
#define BOX_THRESH    SEG_BOX_THRESH
...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "/home/linaro/projects/yolo_test/model/yolov8_seg_labels_list.txt"
...
void matmul_by_npu_fp(std::vector<float> &A_input, float *B_input, float *C_input, int ROWS_A, int COLS_A, int COLS_B, rknn_seg_context_t *app_ctx)
{
    int B_layout = 0;
    int AC_layout = 0;
    int32_t M = ROWS_A;
...
int seg_post_process(rknn_seg_context_t *app_ctx, rknn_output *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, seg_detect_result_list *od_results)
{
    ...
    memset(od_results, 0, sizeof(seg_detect_result_list));
